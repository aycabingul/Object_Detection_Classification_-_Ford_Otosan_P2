...
def json2od(JSON_DIR,json_name):
    ...
    for obj in json_dict["objects"]:# To access each list inside the json_objs list
        ...
        if obj['classTitle']=='Traffic Sign':
                ...
                if (obj['points']['exterior'][1][0] - obj['points']['exterior'][0][0]) < 6 or (obj['points']['exterior'][1][1] - obj['points']['exterior'][0][1]) < 6:
                    continue
...
test_ind  = int(4212 * test_size)#Multiply indices length by test_size and assign it to an int-shaped variable
valid_ind = int(test_ind + 4212 * valid_size)
