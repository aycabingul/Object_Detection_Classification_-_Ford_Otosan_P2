...
def txt_write(jsons,valid_size,test_size):
    ...
    for ln in lines[:test_ind] :
        ...
        copy_image=split_data[0].replace("img","test")
        copyfile(image_path,copy_image)
        txt_path=(split_data[0][:-3]+"txt").replace("img","test")
    ...
    for ln in lines[test_ind:valid_ind]:
        split_data=ln.split("*")
        image_path=split_data[0]
        copy_image=split_data[0].replace("img","valid")
        copyfile(image_path,copy_image)
        txt_path=(split_data[0][:-3]+"txt").replace("img","valid")
        with open(txt_path, 'w') as f:
            for data in split_data[1:]:
                f.write("%s\n" % data)
            f.close()
    ...
    for ln in lines[valid_ind:]:
        split_data=ln.split("*")
        image_path=split_data[0]
        copy_image=split_data[0].replace("img","train")
        copyfile(image_path,copy_image)
        txt_path=(split_data[0][:-3]+"txt").replace("img","train")
        with open(txt_path, 'w') as f:
            for data in split_data[1:]:
                f.write("%s\n" % data)
            f.close()
